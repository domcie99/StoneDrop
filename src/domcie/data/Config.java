...
public class Config {
	...
	public FileConfiguration cfg = stonedrop.getInst().getConfig();
	...
	public void reload(){
		stonedrop.getInst().reloadConfig();
		this.cfg = stonedrop.getInst().getConfig();
		load();
	}
		...
		if(!this.datatype.equalsIgnoreCase("Flat")) {
			if(!this.datatype.equalsIgnoreCase("MySQL")) {
				if(!this.datatype.equalsIgnoreCase("SQLite")) {
					stonedrop.error("Bledny zapis danych: "+Config.getInst().datatype.toUpperCase(), false);
					this.datatype = "flat";
				}
			}
		}
		stonedrop.success("Zapis danych: "+this.datatype.toUpperCase());
		this.debug = cfg.getBoolean("config.debug");
		if(this.debug) stonedrop.success("Debug: "+(this.debug ? "TAK" : "NIE"));
